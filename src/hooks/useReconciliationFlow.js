...
export function useReconciliationFlow() {
  ...
  const [bankCode, setBankCode] = useState(BANK_ACCOUNTS[0]?.bankCode ?? '')
  const [bankAccount, setBankAccount] = useState(BANK_ACCOUNTS[0]?.bankAccount ?? '')
  const [period, setPeriod] = useState(getCurrentPeriod())

  const [bankFile, setBankFile] = useState(null)
  const [systemFile, setSystemFile] = useState(null)

  const [bankUploadStatus, setBankUploadStatus] = useState(STEP_STATUS.IDLE)
  const [systemUploadStatus, setSystemUploadStatus] = useState(STEP_STATUS.IDLE)
  const [runStatus, setRunStatus] = useState(STEP_STATUS.IDLE)

  const [error, setError] = useState(null)

  const periodIsValid = /^\d{4}-\d{2}$/.test(period)
  const contextIsValid = Boolean(bankCode) && Boolean(bankAccount) && periodIsValid
  ...
  const uploadBank = useCallback(
    async (file) => {
      ...
      setError(null)
      try {
        await reconciliationApi.uploadBankFile(file, { bankCode, bankAccount, period })
  ...
  const uploadSystem = useCallback(
    async (file) => {
      ...
      setError(null)
      try {
        await reconciliationApi.uploadSystemFile(file, { bankCode, bankAccount, period })
  ...
  const runReconciliation = useCallback(async () => {
    ...
    setError(null)
    try {
      await reconciliationApi.run({ bankCode, bankAccount, period })
      setContext({ bankCode, bankAccount, period })
  ...
  }, [bankCode, bankAccount, period, setContext])
  ...
  const bothFilesReady =
    bankUploadStatus === STEP_STATUS.DONE && systemUploadStatus === STEP_STATUS.DONE
